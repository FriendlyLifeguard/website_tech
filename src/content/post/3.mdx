...
Aptos was original called Diem, a blockchain project created by Facebook _now Meta_. It aims to solve the existing scalability issue as well provide full modularity of smart contract design that was not offered before.
...
Generally, they can be imported by other modules and lives fully on-chain just like any other smart contracts.
...
These accounts can be created by importing the native 0x1 library from [Aptos](https://aptos.dev/reference/move/?branch=devnet&page=aptos-framework/doc/resource_account.md#0x1_resource_account).
...
This is brings so much flexibility for a smart contract developer as they don't have to keep creating new accounts or keep track of 
accounts that possess as a signer. 

As an example, if an admin account for a module  was compromised for whatever reason, the developer can quickly switch the signer of the module 
...
Data can be passed around by using specific operations for global storages which can be found [here](https://aptos.dev/move/book/global-storage-operators/#global-storage-operators-with-generics).
...
3. Move is very strict on its type and ownership system. Make sure you read the function params carefully and always check if it wants the value directly or its reference. 
